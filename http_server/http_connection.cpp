...
void HttpConnection::start(const std::string& ConnectionData)
{
	readRequest();
	checkDeadline();
	DB.Connect(ConnectionData);
}
...
void HttpConnection::createResponsePost()
{
	...
	if (request_.target() == "/")
	{
		...
		if (DB.IsNoMatches()) {
			...
			std::list<std::string> searchResult;
			DB.PickMostFrequent(searchResult);
